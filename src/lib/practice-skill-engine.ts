...
type EventRow = {
  skill?: string; game_id?: string; xp?: number; accuracy?: number; correct?: boolean; difficulty?: number;
  created_at?: string; metadata?: Record<string, unknown> | null; response_time_ms?: number | null;
};
...
export async function getSkillDashboard(userId: string) {
  ...
  const [profile, events, missions] = await Promise.all([
    ...
    db.from("practice_skill_events").select("skill,game_id,xp,accuracy,correct,difficulty,created_at,metadata,response_time_ms").eq("user_id", userId).order("created_at", { ascending: false }).limit(1000),
  ...
  const allRows = (events.data || []) as EventRow[];
  const freqRows = allRows.filter((row) => {
    const gid = String(row.game_id || "");
    const skill = String(row.skill || "");
    return skill === "ear_training" || gid === "freq-memory" || gid.includes("freq");
  });
  const samples = (freqRows.length ? freqRows : allRows.filter((r) => {
    const m = (r.metadata || {}) as Record<string, unknown>;
    return typeof m.targetHz === "number" || typeof m.guessHz === "number" || typeof m.errorHz === "number";
  })).map((row) => sampleFromEventRow(row));
  ...
  if (effectiveness.enoughEvidence) {
    curriculumFeedbackFa = effectiveness.summaryFa + " " + effectiveness.guidanceFa;
  } else if (frequencySkills.enoughEvidence) {
    curriculumFeedbackFa = focusSuggestion.reasonFa;
  }

  const baseRecommendation = weakest
    ? "تمرکز بعدی: " + weakest.title + " · هدف پیشنهادی " + weakest.recommendedDifficulty + "/500"
    : "از یک تمرین شروع کن تا پروفایل شنیداری ساخته شود.";

  let recommendation = baseRecommendation;
  if (frequencySkills.enoughEvidence && focusSuggestion.focus !== "general") {
    recommendation = focusSuggestion.reasonFa + " · " + baseRecommendation;
  }
  if (effectiveness.enoughEvidence && effectiveness.summaryFa) {
    recommendation = effectiveness.summaryFa + " · " + recommendation;
  }
...
export async function getAdaptivePlan(userId: string) {
  ...
  const focus = dashboard.trainingFocus || "general";
  const preferFreq = focus === "precision" || focus === "consistency" || focus === "range" || focus === "difficulty";
  const slots = [
  ...
  return {
    ...
    exercises: slots.map((skill, i) => {
      ...
      let game = skill === "mixing" ? "eq" : skill === "dynamics" ? "compressor" : skill === "stereo" ? "phase" : skill === "harmony" ? "theory" : skill === "ear_training" ? "tone" : "eq";
      if (preferFreq && skill === "ear_training") game = "freq-memory";
