...
async function extractAudioTagsFromBuffer(buffer: Buffer, storageBasePath: string): Promise<AudioTagPayload> {
  const empty: AudioTagPayload = { artist: "", album: "", genre: "", year: null, duration: null, titleFromTag: "", coverUrl: null };
  ...
  try {
    ...
    if (picture?.data) {
      ...
      const up = await supabase.storage.from(bucket).upload(coverPath, data, { contentType: mime, upsert: true, cacheControl: "31536000" });
      if (!up.error) coverUrl = supabase.storage.from(bucket).getPublicUrl(coverPath).data.publicUrl;
    }
...
export async function registerExistingMedia(input: { publicId: string; title: string; description: string; category: MediaCategory; consent: boolean; mimeType: string; }): Promise<MediaItem> {
  ...
  if (!found) throw new Error("media_object_not_found: فایل در Storage پیدا نشد.");
...
export async function uploadMedia(input: { buffer: Buffer; filename: string; mimeType: string; title: string; description: string; category: MediaCategory; consent: boolean; }): Promise<MediaItem> {
  ...
  const inserted = await supabase.from("media_assets").insert({ storage_path: path, public_url: publicUrl, title: (input.title.trim() || tags?.titleFromTag || input.filename).slice(0, 200), description: input.description.trim().slice(0, 1000), category: input.category, mime_type: input.mimeType, file_ext: ext, consent: input.consent, status: "published", artist: tags?.artist || null, album: tags?.album || null, genre: tags?.genre || null, year: tags?.year || null, duration: tags?.duration || null, cover_url: tags?.coverUrl || null }).select().single();
