...
export async function listHealthyAdminAiModels(models: Array<{ provider_id: string; model_id: string }>) {
  ...
  if (!hasDb()) return models;
  try {
    const result = await db()
      .from("admin_ai_model_health")
      .select(
        "provider_id,model_id,consecutive_failures,success_count,failure_count,cooldown_until,last_error,last_success_at,last_failure_at",
      )
      .in("provider_id", [...new Set(models.map((m) => m.provider_id))]);
    if (result.error) {
      console.error("listHealthyAdminAiModels", result.error.message);
      return models;
    }
    const now = Date.now();
    ...
    console.error("listHealthyAdminAiModels exception", e instanceof Error ? e.message : e);
    return models;
  }
}
      ...
      .upsert(
        {
          ...
          success_count: (existing.data?.success_count || 0) + 1,
          last_success_at: now,
          cooldown_until: null,
          last_error: null,
          updated_at: now,
      ...
      .upsert(
        {
          ...
          cooldown_until: cooldownMs ? new Date(now.getTime() + cooldownMs).toISOString() : null,
          last_error: message,
