...
async function chatOpenAI(provider: AIProvider, model: string, messages: ChatMessage[], signal?: AbortSignal): Promise<string> {
  ...
  const response = await fetch(`${provider.baseUrl}${provider.chatPath || "/chat/completions"}`, { method: "POST", headers, body: JSON.stringify({ model, messages, temperature: 0.3 }), signal });
  const data = await readJson(response);
...
async function chatAnthropic(provider: AIProvider, model: string, messages: ChatMessage[], signal?: AbortSignal): Promise<string> {
  ...
  const response = await fetch(`${provider.baseUrl}${provider.chatPath || "/messages"}`, { method: "POST", headers: authHeaders(provider), body: JSON.stringify({ model, max_tokens: 4096, system: system || undefined, messages: converted }), signal });
  const data = await readJson(response);
...
async function chatGoogle(provider: AIProvider, model: string, messages: ChatMessage[], signal?: AbortSignal): Promise<string> {
  ...
  const response = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents, systemInstruction: system ? { parts: [{ text: system }] } : undefined, generationConfig: { temperature: 0.3 } }), signal });
  const data = await readJson(response);
  if (!response.ok) throw new Error(data?.error?.message || `HTTP ${response.status}`);
