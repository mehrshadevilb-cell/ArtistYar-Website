...
/** Cache resolved channel ids so webhook hot path does not call getChat every time. */
const CHANNEL_ID_CACHE = new Map<string, { expiresAt: number; id: string; username: string }>();

async function resolveConfiguredChannel(): Promise<{ id: string; username: string }> {
  const configured = configuredChannel();
  const cached = CHANNEL_ID_CACHE.get(configured);
  if (cached && cached.expiresAt > Date.now()) return { id: cached.id, username: cached.username };
  ...
  if (configured.startsWith("@")) {
    username = configured;
    try {
      const chat = await tg("getChat", { chat_id: configured });
      id = String(chat?.id || "");
      if (chat?.username) username = "@" + String(chat.username);
    } catch {
      // Keep username-only allowlist when Telegram is temporarily unavailable.
    }
  } else {
    id = configured;
    try {
      const chat = await tg("getChat", { chat_id: configured });
      id = String(chat?.id || id);
      if (chat?.username) username = "@" + String(chat.username);
    } catch {
      // Numeric ids still work without a username.
    }
  }

  CHANNEL_ID_CACHE.set(configured, { expiresAt: Date.now() + 30 * 60 * 1000, id, username });
  return { id, username };
}

function isAllowedPluginDocument(fileName?: string | null, mimeType?: string | null) {
  const name = String(fileName || "").toLowerCase();
  const mime = String(mimeType || "").toLowerCase();
  const allowedExt = [
    ".rar", ".zip", ".7z", ".tar", ".gz", ".tgz", ".bz2", ".xz",
    ".dmg", ".pkg", ".msi", ".exe", ".appimage",
    ".vst", ".vst3", ".component", ".aaxplugin", ".clap", ".dll", ".so", ".dylib",
  ];
  if (allowedExt.some(ext => name.endsWith(ext))) return true;
  if (/application\/(zip|x-zip-compressed|x-rar-compressed|x-7z-compressed|octet-stream|x-tar|gzip)/i.test(mime)) {
    // Accept generic binary only when the filename still looks like a plugin/archive package.
    return /\.(rar|zip|7z|tar|gz|tgz|dmg|pkg|msi|exe|vst3?|component|aaxplugin|clap|dll|so|dylib)$/i.test(name);
  }
  return false;
}

function isChannelAllowed(message: TgMessage, resolved: { id: string; username: string }) {
  const chatId = String(message.chat?.id || "");
  const username = message.chat?.username ? "@" + message.chat.username : "";
  if (resolved.id && chatId && resolved.id === chatId) return true;
  if (resolved.username && username && resolved.username.toLowerCase() === username.toLowerCase()) return true;
  // When Telegram resolution failed and only a @username is configured, require username match.
  const configured = configuredChannel();
  if (configured.startsWith("@") && username) {
    return configured.toLowerCase() === username.toLowerCase();
  }
  if (!configured.startsWith("@") && chatId) {
...
async function identify(imageFileId: string | null, fileName: string, caption: string) {
  ...
  try {
    if (!imageFileId) throw new Error("telegram_photo_missing");
  ...
  // AI is an enhancement, not a publication dependency. Keep the post
  // publishable with conservative facts from the filename and source caption.
  const fallbackTitle = titleFromFileName(fileName) || clean(caption.split("\n")[0], 160) || "پلاگین بدون نام";
  ...
  return {
    data: {
      ...
      description: clean(caption, 1800),
      features: [],
      tags: [],
      translatedCaption: sanitizeCaption(caption),
    },
    provider: "fallback",
    model: "rules",
  } satisfies AiResult;
...
export async function processPluginPair(photo: TgMessage | null, doc: TgMessage) {
  if (!db) throw new Error("supabase_not_configured");
  const chatId = String(photo?.chat?.id || doc.chat?.id || "");
  const photoFileId = photo?.photo?.at(-1)?.file_id || doc.document?.thumbnail?.file_id || null;
  const docFileId = doc.document?.file_id;
  if (!chatId || !docFileId) throw new Error("plugin_pair_missing_media");
  ...
  const linkMessageId = doc.message_id || photo?.message_id || 0;
  ...
  const result = await db.from("telegram_plugin_posts").upsert({
    channel_id: chatId, photo_message_id: photo?.message_id || null, document_message_id: doc.message_id,
  ...
  try {
    ...
    if (photoFileId) await syncPublishedPluginCover({
  ...
  try {
    const sourceMessage = photo || doc;
    await editCaption(sourceMessage.chat?.id || chatId, sourceMessage.message_id, finalCaption);
  } catch (error) {
    const message = clean(error instanceof Error ? error.message : String(error), 300);
...
export async function enqueuePluginMessage(message: TgMessage) {
  ...
  if (!kind) return { ignored: true, reason: "unsupported_media" };

  // Ignore temporary recovery posts created by the 404 CDN fallback.
  if ((message.caption || "").includes("ARTISTYAR_INTERNAL_RECOVERY")) {
    return { ignored: true, reason: "internal_recovery" };
  }

  if (kind === "document" && !isAllowedPluginDocument(message.document?.file_name, message.document?.mime_type)) {
    return { ignored: true, reason: "document_type_rejected" };
  }
  ...
  // Queue rows are deleted after success, so also dedupe against the durable
  // catalog. This closes the delayed-webhook retry window without preventing
  // an operator from re-queuing a failed/hidden post through the admin route.
  const existingPost = kind === "document"
    ? await db.from("telegram_plugin_posts").select("id,status").eq("channel_id", chatId).eq("telegram_file_id", fileId).in("status", ["processing", "published"]).limit(1).maybeSingle()
    : await db.from("telegram_plugin_posts").select("id,status").eq("channel_id", chatId).eq("telegram_photo_file_id", fileId).in("status", ["processing", "published"]).limit(1).maybeSingle();
  if (existingPost.error) throw new Error("plugin_post_dedupe_check_failed:" + existingPost.error.message);
  if (existingPost.data) return { ignored: true, duplicate: true, original_post_id: String(existingPost.data.id) };

  const payload = {
    channel_id: chatId,
    channel_username: channelUsername,
    message_id: message.message_id,
    media_group_id: message.media_group_id ? String(message.media_group_id) : null,
    kind,
    file_id: fileId,
    file_name: message.document?.file_name || null,
    mime_type: message.document?.mime_type || null,
    file_size: message.document?.file_size || null,
    thumbnail_file_id: message.document?.thumbnail?.file_id || null,
    caption: clean(message.caption, 1200),
  };

  const inserted = await db.from("telegram_plugin_ingest_queue").upsert(payload, {
    onConflict: "channel_id,message_id",
    ignoreDuplicates: true,
  }).select("*").maybeSingle();

  if (inserted.error) {
    // Race on the (channel_id, kind, file_id) unique index is expected under webhook retries.
    if (/duplicate key|unique constraint|23505/i.test(inserted.error.message)) {
      return { ignored: true, duplicate: true, reason: "file_unique_race" };
    }
...
function rowToPhoto(row: any): TgMessage {
  const username = String(row.channel_username || "").replace(/^@/, "") || undefined;
  return {
    message_id: Number(row.message_id),
    media_group_id: row.media_group_id ? String(row.media_group_id) : undefined,
    chat: { id: Number(row.channel_id), username },
    caption: row.caption || "",
...
function rowToDocument(row: any): TgMessage {
  ...
  return {
    ...
    document: {
      ...
      thumbnail: row.thumbnail_file_id ? { file_id: row.thumbnail_file_id, width: 1, height: 1 } : undefined,
    },
  };
}
...
export async function processPendingPluginPairs(limit = 5) {
  ...
  const isFree = (row: any) => (!row.processing_at || String(row.processing_at) <= staleBefore) && (!row.next_attempt_at || String(row.next_attempt_at) <= new Date().toISOString());

  const docsByChannel = new Map<string, any[]>();
  for (const row of documents.data || []) {
    if (!isFree(row)) continue;
  ...
  for (const photo of photos.data || []) {
    ...
    if (!isFree(photo)) continue;

    const channelDocs = (docsByChannel.get(String(photo.channel_id)) || []).filter(d => !usedDocIds.has(String(d.id)));
    const photoTime = new Date(photo.received_at).getTime();
    const photoMsg = Number(photo.message_id);
    const photoGroup = photo.media_group_id ? String(photo.media_group_id) : "";

    // Prefer same media_group_id, then nearest message_id, then nearest timestamp within 5 minutes.
    let best: any = null;
    let bestScore = Number.POSITIVE_INFINITY;
    for (const doc of channelDocs) {
      const docGroup = doc.media_group_id ? String(doc.media_group_id) : "";
      const distanceMs = Math.abs(new Date(doc.received_at).getTime() - photoTime);
      if (distanceMs > 300000 && !(photoGroup && docGroup && photoGroup === docGroup)) continue;

      let score = distanceMs;
      if (photoGroup && docGroup && photoGroup === docGroup) score -= 1_000_000;
      score += Math.abs(Number(doc.message_id) - photoMsg) * 10;
      if (score < bestScore) {
        best = doc;
        bestScore = score;
      }
    ...
    try {
      ...
      try {
        await db.from("telegram_plugin_ingest_queue")
          .update({ processing_at: null, last_error: clean(error instanceof Error ? error.message : error, 500), last_attempt_at: new Date().toISOString(), next_attempt_at: new Date(Date.now() + 60000).toISOString() })
          .in("id", [photo.id, best.id]);
      } catch {
        try { await db.from("telegram_plugin_ingest_queue").update({ processing_at: null }).in("id", [photo.id, best.id]); } catch {}
      }
      errors.push({
  ...
  // Process documents that have a Telegram thumbnail or are standalone posts.
  // Delaying five minutes preserves the photo+document pairing window.
  for (const doc of documents.data || []) {
    if (processed >= safeLimit || usedDocIds.has(String(doc.id)) || !isFree(doc)) continue;
    if (Date.now() - new Date(doc.received_at).getTime() < 300000) continue;
    const claim = await db.rpc("claim_telegram_plugin_item", { p_queue_id: doc.id });
    if (claim.error) { errors.push({ photo_message_id: 0, document_message_id: Number(doc.message_id), error: "plugin_item_claim_failed:" + claim.error.message }); continue; }
    if (!claim.data) continue;
    try {
      await processPluginPair(null, rowToDocument(doc));
      const deleted = await db.from("telegram_plugin_ingest_queue").delete().eq("id", doc.id);
      if (deleted.error) throw new Error("plugin_queue_cleanup_failed:" + deleted.error.message);
      processed++;
    } catch (error) {
      const message = clean(error instanceof Error ? error.message : error, 500);
      await db.from("telegram_plugin_ingest_queue").update({ processing_at: null, last_error: message, last_attempt_at: new Date().toISOString(), next_attempt_at: new Date(Date.now() + 60000).toISOString() }).eq("id", doc.id);
      errors.push({ photo_message_id: 0, document_message_id: Number(doc.message_id), error: message });
    }
  }
  return { processed, errors, pending_checked: (photos.data || []).length + (documents.data || []).length };
}
