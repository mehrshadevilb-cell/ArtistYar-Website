...
export function PracticeGameSession({
    ...
    (lvl: number, idx: number) => {
      ...
      if (gameId === "freq-memory") {
        ...
        exerciseType = selectFreqExercise({
          profile: null,
          focus: plan?.focus || trainingFocusRef.current,
      ...
      if (gameId === "freq-memory" && typeof r.targetHz === "number") {
  ...
  const startSession = () => {
    ...
    if (gameId === "freq-memory") {
      ...
      setSessionPlan(plan);
      trainingFocusRef.current = plan.focus;
    } else {
          ...
          <div className="flex items-center justify-between text-[11px] text-ink-500">
            <span>
              ...
              {gameId === "freq-memory" && activeExercise !== "general"
                ? ` · ${activeExercise === "precision" ? "دقت" : activeExercise === "range" ? "گستره" : activeExercise === "stability" ? "پایداری" : activeExercise === "challenge" ? "چالش" : ""}`
                : ""}
            </span>
