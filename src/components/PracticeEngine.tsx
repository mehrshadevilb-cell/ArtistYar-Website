...
function PracticeLabLoading({ label = "در حال بارگذاری تمرین…" }: { label?: string }) {
  return (
    <div className="container-ay flex min-h-[50vh] flex-col items-center justify-center gap-3 py-16 text-center" dir="rtl" role="status" aria-live="polite">
      <div className="h-8 w-8 animate-spin rounded-full border-2 border-amber-300/30 border-t-amber-300" aria-hidden />
      <p className="text-sm text-sand-50">{label}</p>
      <p className="text-xs text-ink-500">اگر چند ثانیه طول کشید، اتصال را بررسی کن.</p>
    </div>
  );
}

const CoreEarGym = dynamic(() => import("@/components/CoreEarGym").then((m) => m.CoreEarGym), { ssr: false, loading: () => <PracticeLabLoading /> });
const TheoryLab = dynamic(() => import("@/components/TheoryLab").then((m) => m.TheoryLab), { ssr: false, loading: () => <PracticeLabLoading /> });
const ProArcadeLab = dynamic(() => import("@/components/ProArcadeLab").then((m) => m.ProArcadeLab), { ssr: false, loading: () => <PracticeLabLoading /> });
const DailyVoicingLab = dynamic(() => import("@/components/DailyVoicingLab").then((m) => m.DailyVoicingLab), { ssr: false, loading: () => <PracticeLabLoading /> });
const AdaptiveWorkoutLab = dynamic(() => import("@/components/AdaptiveWorkoutLab").then((m) => m.AdaptiveWorkoutLab), { ssr: false, loading: () => <PracticeLabLoading /> });
const SoundGymLab = dynamic(() => import("@/components/SoundGymLab").then((m) => m.SoundGymLab), { ssr: false, loading: () => <PracticeLabLoading /> });
const UserAudioLab = dynamic(() => import("@/components/UserAudioLab").then((m) => m.UserAudioLab), { ssr: false, loading: () => <PracticeLabLoading /> });
const PracticeProfileLab = dynamic(() => import("@/components/PracticeProfileLab").then((m) => m.PracticeProfileLab), { ssr: false, loading: () => <PracticeLabLoading /> });
const DailyChallengeLab = dynamic(() => import("@/components/DailyChallengeLab").then((m) => m.DailyChallengeLab), { ssr: false, loading: () => <PracticeLabLoading /> });
  ...
  useEffect(() => {
    ...
    void fetch("/api/practice/status", { credentials: "include" })
      .then(async (r) => {
        ...
        if (cancelled) return;
        if (d?.ok || d?.pro != null || typeof d?.used === "number") {
          setStatus({
            pro: Boolean(d.pro ?? d.unlimited),
            remaining: d.remaining ?? null,
            used: d.used,
            dailyLimit: d.dailyLimit ?? 5,
          });
        } else {
          setStatus({ pro: false, remaining: 5, used: 0, dailyLimit: 5 });
        }
      })
      .catch(() => {
        if (!cancelled) setStatus({ pro: false, remaining: 5, used: 0, dailyLimit: 5 });
      });
    return () => {
      cancelled = true;
    };
