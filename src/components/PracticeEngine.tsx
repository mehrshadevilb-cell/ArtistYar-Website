    ...
    <section className="mt-10">
      ...
      <div className="card-ay mt-5 p-6 sm:p-10">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {sourceLabel ? (
            <span className="rounded-full border border-white/10 px-3 py-1 text-[11px] text-ink-400">{sourceLabel}</span>
          ) : null}
        </div>

        {loading && <p className="mt-8 text-sm text-ink-500">در حال آماده‌سازی سوال غیرتکراری…</p>}

        {!loading && q && (
          <>
            <p className="mt-6 text-base leading-8 text-sand-50">{q.prompt}</p>
            {q.hint ? <p className="mt-2 text-xs leading-6 text-ink-500">راهنما: {q.hint}</p> : null}
