    ...
    <header className="site-header sticky top-0 z-50">
      <div className="container-ay flex h-[76px] items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-2 md:flex">
          {links.map((link) => <Link key={link.href} href={link.href} className={`nav-link ${isHome && link.href.includes("#") ? "" : ""}`}>{link.label}</Link>)}
        </nav>
        ...
        <button type="button" aria-label={open ? "بستن منو" : "باز کردن منو"} aria-expanded={open} className="menu-button md:hidden" onClick={() => setOpen(v => !v)}>{open ? <X size={20} /> : <Menu size={20} />}</button>
      </div>
      <div className={`mobile-menu md:hidden ${open ? "mobile-menu-open" : ""}`} aria-hidden={!open}>
