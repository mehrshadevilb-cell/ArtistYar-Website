...
export default function MusicAnalyzerLab() {
    ...
    try {
      ...
      if (data?.ok) {
        setQuota({
          ...
          tier: String(
            data.tier || (data.admin ? "admin" : data.pro ? "pro" : data.course ? "course" : "free"),
          ),
        });
      }
    } catch {
  ...
  const analyze = async () => {
    ...
    if (!isAdmin && !quota.pro && quota.remaining <= 0) {
    ...
    try {
      ...
      if (data.quota) {
        setQuota({
          ...
          course: Boolean(data.quota.course),
          admin: Boolean(data.quota.admin),
          tier: String(data.quota.tier || "free"),
        });
  ...
  const canAnalyze = Boolean(
    file && metrics && !measuring && !loading && (isAdmin || quota.pro || quota.remaining > 0),
  );
    ...
    <section className="mt-2">
      ...
      <div className="mt-5 overflow-hidden rounded-3xl border border-cyan-400/25 bg-gradient-to-br from-cyan-400/[.1] via-white/[.03] to-violet-400/[.08] p-6 sm:p-10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-7 text-ink-300">
              فایل را آپلود کن. عددهای واقعی Peak، RMS، Crest، استریو و انرژی پایین/میانی/بالا گرفته می‌شود.
              تحلیل سخت‌گیرانه مثل Reference می‌دهد: EQ، کمپرس، لیمیتر و نقشهٔ قدم‌به‌قدم با فارسی ساده و قابل‌اجرا.
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-right">
            ...
            <div className="mt-1 text-lg font-semibold text-cyan-100">
              {isAdmin || quota.admin || quota.pro ? "∞" : `${quota.remaining} / ${quota.limit}`}
            </div>
          ...
          <div className="space-y-4">
            ...
            <label className="block text-xs text-ink-400">
              ...
              <textarea
                ...
                placeholder="مثلاً: وکال عقب است، بیس زیاد، میکس کدر..."
              />
            </label>
          </div>
        </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {(
              [
                ...
                [
                  "Stereo Corr.",
                  metrics.stereoCorrelation == null ? "N/A" : metrics.stereoCorrelation.toFixed(2),
                  Sparkles,
                ],
              ] as const
        ...
        <button type="button" disabled={!canAnalyze} onClick={() => void analyze()} className="btn-primary mt-6">
          {loading ? (
            <>
              <Loader2 size={16} className="animate-spin" /> در حال تحلیل سخت‌گیرانه…
            </>
        ...
        {!isAdmin && !quota.pro && quota.remaining === 0 ? (
          <p className="mt-3 text-xs text-amber-200">
            سهمیه امروز تمام شد. رایگان: ۱ بار · هنرجوی آموزش: ۵ بار · Pro: نامحدود تا پایان اشتراک.
          </p>
        ) : null}
        {isAdmin ? <p className="mt-3 text-xs text-emerald-200">ادمین: بدون محدودیت روزانه.</p> : null}
        {!isAdmin && quota.pro ? (
          <p className="mt-3 text-xs text-emerald-200">اشتراک Pro فعال است — تحلیل نامحدود تا پایان اشتراک.</p>
        ) : null}
        {!isAdmin && !quota.pro && quota.course ? (
          <p className="mt-3 text-xs text-cyan-200">هنرجوی آموزش: روزانه تا ۵ تحلیل.</p>
        ) : null}
        ...
        <div className="mt-6 space-y-4">
          <div className="card-ay p-6 sm:p-8">
            ...
            {analysis.descriptors ? (
              <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {(
                  [
                    ["تونال", analysis.descriptors.tonal, Waves],
                    ["استریو", analysis.descriptors.stereo, BarChart3],
                    ["داینامیک", analysis.descriptors.dynamics, Zap],
                    ["بلندی", analysis.descriptors.loudness, Gauge],
                  ] as const
                ).map(([label, value, Icon]) => (
                  <div key={label} className="rounded-xl border border-white/10 bg-white/[.03] p-4">
                    <div className="flex items-center gap-2 text-xs text-ink-500">
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <div className="card-ay p-6">
              <h3 className="text-base text-sand-50">بلندی و داینامیک</h3>
            ...
            <div className="card-ay p-6">
              <h3 className="text-base text-sand-50">تعادل تونال</h3>
              <p className="mt-2 text-sm leading-7 text-ink-300">{analysis.tonal.summary}</p>
              <div className="mt-4 space-y-3">
                <EnergyBar label="پایین" value={analysis.tonal.low} color="bg-amber-400/80" />
                <EnergyBar label="میانی" value={analysis.tonal.mid} color="bg-cyan-400/80" />
                <EnergyBar label="بالا" value={analysis.tonal.high} color="bg-violet-400/80" />
              </div>
            </div>
            <div className="card-ay p-6">
              <h3 className="text-base text-sand-50">استریو</h3>
              <p className="mt-2 text-2xl text-cyan-100">
                {analysis.stereo.correlation?.toFixed(2) ?? "N/A"}
              </p>
          ...
          {analysis.compression ? (
            <div className="card-ay p-6 sm:p-8">
              <h3 className="text-base text-sand-50">کمپرس</h3>
          ...
          <div className="card-ay p-6 sm:p-8">
            <h3 className="flex items-center gap-2 text-base text-sand-50">
              <Target size={18} className="text-gold-300" /> پیشنهاد EQ
            </h3>
            <ul className="mt-4 grid gap-2 sm:grid-cols-2">
              {analysis.eq.map((x, i) => (
                <li
                  key={i}
                  className="rounded-lg border border-white/10 bg-white/[.02] px-4 py-3 text-sm leading-6 text-ink-300"
                >
                  {x}
                </li>
              ))}
          ...
          {analysis.mixBalance?.length ? (
            <div className="card-ay p-6 sm:p-8">
              <h3 className="text-base text-sand-50">تعادل اجزای میکس</h3>
          ...
          <div className="card-ay p-6 sm:p-8">
            <h3 className="flex items-center gap-2 text-base text-sand-50">
              <CheckCircle2 size={18} className="text-emerald-300" /> نقشهٔ کار
            </h3>
          ...
          {(analysis.quickFixes?.length || analysis.referenceTips) && (
            <div className="card-ay p-6">
              {analysis.quickFixes?.length ? (
                <>
                  <h3 className="text-base text-sand-50">رفع سریع</h3>
                  <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                    {analysis.quickFixes.map((q) => (
                      <li
                        key={q}
                        className="rounded-lg border border-white/10 bg-white/[.02] px-3 py-2 text-xs text-ink-300"
                      >
                        {q}
                      </li>
                    ))}
