...
/** استخراج متریک واقعی از بافر صوتی — هر فایل باید عدد متفاوت بدهد */
async function measureAudio(file: File): Promise<Metrics> {
  ...
  try {
    ...
    // Peak / TruePeak / RMS / Stereo / Clip — dense sampling so peak is not missed
    const targetPoints = 2_500_000;
    const stride = Math.max(1, Math.floor(totalLen / targetPoints));
    ...
    for (let i = 0; i < totalLen; i += stride) {
      ...
      if (al > peak) peak = al;
      if (ar > peak) peak = ar;
      if (i + stride < totalLen) {
        const l2 = left[i + stride] ?? 0;
        const r2 = right[i + stride] ?? l2;
        const midL = Math.abs((l + l2) * 0.5);
        const midR = Math.abs((r + r2) * 0.5);
        if (midL > truePeak) truePeak = midL;
        if (midR > truePeak) truePeak = midR;
    ...
    if (count < 10) throw new Error("نمونه‌برداری کافی از فایل انجام نشد.");
    ...
    const clipPct = (clipCount / count) * 100;
    ...
    // Spectral bands — frames spread across whole track
    const N = 2048;
    ...
    const lowEnergyPct = pct(bands.sub + bands.low);
    const midEnergyPct = pct(bands.lowMid + bands.mid + bands.presence);
    const highEnergyPct = pct(bands.high + bands.air);
    const spectralCentroidHz = energy > 0 ? weighted / energy : null;
    const approxLufs = rmsDbfs - 0.691 + (highEnergyPct > 45 ? 1.2 : highEnergyPct < 20 ? -0.8 : 0);
          ...
          <div className="mt-4 space-y-3 rounded-2xl border border-white/10 bg-black/15 p-4">
            ...
            {metrics.bandEnergy ? (
              <div className="grid gap-2 sm:grid-cols-4 border-t border-white/5 pt-3">
                ...
                <EnergyBar label="Low" value={metrics.bandEnergy.low} color="bg-amber-400/60" />
                <EnergyBar label="Low-Mid" value={metrics.bandEnergy.lowMid} color="bg-cyan-400/60" />
                <EnergyBar label="Mid" value={metrics.bandEnergy.mid} color="bg-cyan-300/60" />
                <EnergyBar label="Presence" value={metrics.bandEnergy.presence} color="bg-violet-300/60" />
                <EnergyBar label="High" value={metrics.bandEnergy.high} color="bg-violet-400/60" />
                <EnergyBar label="Air" value={metrics.bandEnergy.air} color="bg-fuchsia-400/60" />
              </div>
            ) : null}
          </div>
        ) : null}
