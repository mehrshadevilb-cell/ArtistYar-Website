      ...
      <div className="mt-5 overflow-hidden rounded-3xl border border-cyan-400/25 bg-gradient-to-br from-cyan-400/[.1] via-white/[.03] to-violet-400/[.08] p-6 sm:p-10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-7 text-ink-300">فایل خودت را آپلود کن. می‌توانی رفرنس هم بگذاری تا AI ترک را با همان رفرنس مقایسه کند. Peak، RMS، Crest، استریو و انرژی واقعی گرفته می‌شود.</p>
          </div>
          ...
          <div>
            ...
            <div className="mt-3">
              <button type="button" onClick={() => refInputRef.current?.click()} className="flex w-full items-center gap-3 rounded-xl border border-dashed border-violet-300/30 bg-black/15 px-4 py-3 text-right transition hover:border-violet-200/50 hover:bg-violet-400/[.06]">
                ...
                {refFile ? (
                  <span role="button" tabIndex={0} onClick={(e) => { e.stopPropagation(); setRefFile(null); setRefMetrics(null); if (refInputRef.current) refInputRef.current.value = ""; }} className="text-[11px] text-ink-400 hover:text-red-300">حذف</span>
                ) : null}
              </button>
        ...
        {metrics ? (
          <p className="mt-2 text-[10px] leading-5 text-ink-500">* ≈ LUFS تقریبی است (K-weight سبک). برای عدد رسمی از Youlean استفاده کن. اگر رفرنس آپلود کنی، امتیاز بر اساس نزدیکی به همان رفرنس است.</p>
        ) : null}
        {metrics ? (
        ...
        <button type="button" disabled={!canAnalyze} onClick={() => void analyze()} className="btn-primary mt-6">
          {loading ? (<><Loader2 size={16} className="animate-spin" /> در حال تحلیل سخت‌گیرانه…</>) : !isAdmin && !quota.pro && quota.remaining <= 0 ? (<><LockKeyhole size={16} /> سهمیه امروز تمام شده</>) : (<><Sparkles size={16} /> {refMetrics ? "مقایسه با رفرنس" : "Analyze Music"}</>)}
        </button>
          ...
          <div className="card-ay p-6 sm:p-8">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-xl text-sand-50">{refMetrics ? "مقایسه با رفرنس تو" : "نتیجه تحلیل فایل"}</h2>
              ...
              {typeof analysis.matchScore === "number" ? (
                <div className="rounded-2xl border border-cyan-400/25 bg-cyan-400/10 px-5 py-3 text-center">
                  <p className="text-[10px] text-cyan-200/80">{refMetrics ? "نزدیکی به رفرنس تو" : "نزدیکی به رفرنس"}</p>
