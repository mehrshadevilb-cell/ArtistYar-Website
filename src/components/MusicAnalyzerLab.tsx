...
async function measureAudio(file: File): Promise<Metrics> {
  ...
  try {
    ...
    // RMS + Stereo + K-weighted energy for ≈LUFS
  ...
  return (
    <section className="mt-2">
      ...
      <div className="mt-5 overflow-hidden rounded-3xl border border-cyan-400/25 bg-gradient-to-br from-cyan-400/[.1] via-white/[.03] to-violet-400/[.08] p-6 sm:p-10">
        ...
        {metrics ? (
          <p className="mt-2 text-[10px] leading-5 text-ink-500">* ≈ LUFS تقریبی است (K-weight سبک، نزدیک به Integrated). برای عدد رسمی از Youlean استفاده کن. RMS اینجا mid-channel است.</p>
        ) : null}
        {metrics ? (
