  ...
  const loadAdaptive = useCallback(async () => {
    ...
    try {
      const p = await withTimeout(
        fetch("/api/practice/adaptive?userId=" + encodeURIComponent(user.id), {
          cache: "no-store",
          credentials: "include",
        }).then((r) => r.json()),
        8000,
      ...
      <div className="p-5 sm:p-7">
        {loading || !q ? (
          ...
          <>
            ...
            {submitError ? (
              <p className="mt-3 rounded-xl border border-amber-400/20 bg-amber-400/[.05] p-3 text-xs leading-6 text-amber-100">
                {submitError}
              </p>
            ) : null}
