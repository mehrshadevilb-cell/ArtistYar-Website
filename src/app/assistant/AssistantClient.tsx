...
export default function AssistantPage() {
  ...
  function stopGeneration() {
    ...
    setMessages((current) => {
      ...
      if (last?.role === "assistant" && !last.error && !last.stopped) return current;
      return [
        ...current,
        {
          id: Date.now() + 1,
          role: "assistant",
          text: "تولید پاسخ متوقف شد.",
          stopped: true,
          retryText: [...current].reverse().find((m) => m.role === "user")?.text || undefined,
        },
      ];
    });
  ...
  async function sendMessage(message: string, opts?: { regenerateFromId?: number }) {
    ...
    if (opts?.regenerateFromId != null) {
      const idx = messages.findIndex((m) => m.id === opts.regenerateFromId);
      ...
      const res = await fetch("/api/ai/chat", {
        ...
        body: JSON.stringify({
          ...
          messages: nextMessages
            .filter((item) => !item.error && !item.stopped)
        ...
        >
          {isEmpty ? (
            ...
            messages.map((message) => (
              ...
              >
                ...
                <div className="min-w-0 max-w-[90%] sm:max-w-[82%]">
                  <div
                  ...
                  >
                    {message.role === "assistant" && !message.error && !message.stopped ? (
                      <MarkdownContent text={message.text} />
                    ) : (
                      <p className="leading-7 break-words">{message.text}</p>
                    )}
                  </div>
                  <div
                  ...
                  {(message.error || message.stopped) && message.retryText ? (
                    <button
                      type="button"
                      onClick={() =>
                        sendMessage(message.retryText || "", { regenerateFromId: message.id })
                      }
                      disabled={busy}
                    ...
                    >
                      ...
                      {message.stopped ? "ادامه / دوباره" : "تلاش دوباره"}
                    </button>
                  ) : null}
                </div>
              </div>
          ...
          {busy ? (
            <div className="assistant-msg flex gap-2.5">
              ...
              <div className="flex items-center gap-3 rounded-2xl rounded-tl-md bg-white/[0.04] px-3.5 py-3 text-sm text-ink-400">
                ...
                <button
                  ...
                  className="mr-auto inline-flex items-center gap-1 rounded-lg border border-white/10 px-2 py-1 text-[11px] text-ink-300 transition hover:border-red-400/40 hover:text-red-300"
                  aria-label="توقف تولید پاسخ"
                >
                  <SafeIcon name="Square" size={11} aria-hidden="true" />
                  توقف
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between gap-2 px-2.5 pb-2.5 sm:px-3 sm:pb-3">
              ...
              {busy ? (
                <button
                  type="button"
                  onClick={stopGeneration}
                  className="inline-flex items-center gap-1.5 rounded-xl border border-red-400/30 bg-red-400/10 px-3.5 py-2 text-xs font-medium text-red-300 transition hover:bg-red-400/15"
                >
                  <SafeIcon name="Square" size={14} aria-hidden="true" />
                  توقف
                </button>
              ) : (
                <button
