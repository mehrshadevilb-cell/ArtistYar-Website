...
export default function CoursesPage() {
  ...
  return <section className="container-ay py-16">
    ...
    {selected ? <div className="card-ay mx-auto mt-12 max-w-lg p-7"><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-medium text-sand-50">شروع مسیر: {selected.title}</h3><StatusChip tone="gold">پرداخت کارت‌به‌کارت</StatusChip></div><p className="mt-2 text-xs leading-6 text-ink-500">مشخصاتت را وارد کن، سپس شماره کارت مقصد و امکان بارگذاری رسید را همین‌جا می‌بینی. بررسی نهایی توسط تیم آرتیست‌یار انجام می‌شود.</p><form className="mt-5 space-y-3" onSubmit={onOrder}><input className="input-ay" name="full_name" placeholder="نام کامل…" autoComplete="name" required /><input className="input-ay" name="phone" type="tel" inputMode="tel" placeholder="09xxxxxxxxx…" autoComplete="tel" required /><input className="input-ay" name="note" placeholder="الان روی چه چیزی کار می‌کنی؟ (اختیاری)…" /><div className="flex gap-2"><button type="submit" className="btn-primary flex-1" disabled={busy}>{busy ? "در حال ثبت…" : "نمایش اطلاعات پرداخت"}</button><button type="button" className="btn-ghost" onClick={() => setSelected(null)}>بستن</button></div></form>{msg ? <div className="mt-5 rounded-2xl border border-gold-500/20 bg-gold-500/[.06] p-4" aria-live="polite"><pre className="whitespace-pre-wrap text-xs leading-6 text-gold-300">{msg}</pre>{cardNumber ? <div className="mt-4 rounded-2xl border border-gold-400/20 bg-gradient-to-br from-gold-400/[.12] via-black/20 to-amber-950/20 p-3 shadow-[0_10px_30px_rgba(214,166,74,.08)]"><div className="flex items-center justify-between gap-3"><p className="text-xs font-medium text-gold-200">شماره کارت مقصد</p>{cardHolder ? <span className="text-[11px] text-ink-400">{cardHolder}</span> : null}</div><div className="mt-3 flex items-center gap-2 rounded-xl border border-white/10 bg-black/25 p-1.5 pl-2"><code dir="ltr" className="flex-1 px-2 text-sm font-medium tracking-[.12em] text-sand-50">{cardNumber}</code><button type="button" aria-label="کپی شماره کارت" className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium shadow-sm transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-300/70 ${cardCopied ? "bg-emerald-400/15 text-emerald-300 ring-1 ring-emerald-300/30" : "bg-gold-300 text-[#211b10] hover:bg-gold-200 hover:shadow-[0_6px_18px_rgba(240,194,92,.25)] active:scale-95"}`} onClick={() => { void navigator.clipboard.writeText(cardNumber).then(() => { setCardCopied(true); window.setTimeout(() => setCardCopied(false), 1800); }); }}>{cardCopied ? <Check size={14} strokeWidth={2.5} /> : <Copy size={14} strokeWidth={2.2} />}{cardCopied ? "کپی شد" : "کپی شماره"}</button></div></div> : null}</div> : null}{paymentId ? <form className="mt-5 space-y-3 border-t border-white/[.07] pt-5" onSubmit={onReceipt}><div><p className="text-sm font-medium text-sand-50">رسید پرداخت را همین‌جا ارسال کن</p><p className="mt-1 text-xs leading-6 text-ink-500">فرمت‌های مجاز: تصویر یا PDF، حداکثر ۱۰ مگابایت.</p></div><input className="block w-full rounded-xl border border-white/10 bg-black/20 p-3 text-xs text-ink-300 file:mr-3 file:rounded-lg file:border-0 file:bg-gold-500/15 file:px-3 file:py-2 file:text-gold-300" type="file" accept="image/*,.pdf,application/pdf" required onChange={(e) => setReceipt(e.target.files?.[0] || null)} /><button type="submit" className="btn-primary w-full" disabled={receiptBusy || !receipt}>{receiptBusy ? "در حال ارسال رسید…" : "ارسال رسید برای بررسی"}</button>{receiptMsg ? <p className="rounded-xl bg-emerald-500/10 p-3 text-xs leading-6 text-emerald-300" aria-live="polite">{receiptMsg}</p> : null}</form> : null}{paymentId ? <Link href={`/track?payment_id=${paymentId}`} className="mt-4 inline-flex text-xs text-sand-100 underline decoration-gold-500/60 underline-offset-4 hover:text-gold-300">پیگیری وضعیت سفارش</Link> : null}</div> : null}
  </section>;
}
