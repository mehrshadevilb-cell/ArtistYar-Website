...
export default function CommercePage(){
 ...
 <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["پرداخت‌ها","/admin/payments"],["رزروها","/admin/reservations"],["اشتراک Practice","/admin/practice/subscriptions"],["گزارش مالی","/admin/analytics"]].map(([t,h])=><Link key={h} href={h} className="card-ay p-5 hover:border-gold-500/30"><p className="font-medium text-sand-50">{t}</p><p className="mt-2 text-xs text-ink-500">باز کردن ماژول canonical ←</p></Link>)}</div>
 </div>}
