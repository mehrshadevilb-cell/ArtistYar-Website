  ...
  try {
    ...
    throw new Error(
      response.status === 413
        ...
        : `پاسخ نامعتبر از سرور دریافت شد (${response.status}). احتمالاً deploy جدید هنوز فعال نشده است.`,
    );
  }
}
...
export default function AdminMediaPage() {
  ...
  async function loadItems() {
    try {
      ...
      setError(listError instanceof Error ? listError.message : "دریافت محتوای قبلی ناموفق بود.");
    }
  }

  async function loadStorage(currentToken = token) {
    try {
      const response = await fetch("/api/media?source=storage", {
        headers: { "x-artistyar-admin-token": currentToken },
      });
      const data = await readApiResponse(response);
      if (!response.ok) throw new Error(data.error || "دریافت فایل‌های Storage ناموفق بود.");
      setStorageItems(data.items || []);
    } catch (storageError) {
      setError(storageError instanceof Error ? storageError.message : "دریافت فایل‌های Storage ناموفق بود.");
    }
  }
  ...
  async function registerStorageItem(item: StorageItem) {
    ...
    try {
      const response = await fetch("/api/media", {
        method: "PUT",
        headers: { "Content-Type": "application/json", "x-artistyar-admin-token": token },
        body: JSON.stringify({
          action: "register",
          publicId: item.path,
          title,
          description: "",
          category,
          consent: category !== "student-work" || true,
      ...
      setError(registerError instanceof Error ? registerError.message : "ثبت فایل ناموفق بود.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function refreshTags(item: MediaItem) {
    ...
    try {
      ...
      setError(refreshError instanceof Error ? refreshError.message : "استخراج تگ ناموفق بود.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/media", {
        method: "POST",
        headers: { "x-artistyar-admin-token": token },
        body: form,
      });
      const data = await readApiResponse(response);
  ...
  async function saveEdit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(editError instanceof Error ? editError.message : "ویرایش ناموفق بود.");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm leading-7 text-ink-400">
          فایل‌ها در Supabase Storage ذخیره می‌شوند. دستهٔ <strong className="text-sand-100">ProdBy Mehrshad</strong> برای
          نمونه‌کارهای خودت (فولدر ProdBy Mehrshad) جدا از هنرجو است. کاور MP3 از خود فایل استخراج می‌شود.
        </p>
      </div>
      <div className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
        <form className="card-ay space-y-4 p-6" onSubmit={onSubmit}>
          <div className="flex items-center gap-3 border-b border-white/[.07] pb-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-ink-500">حداکثر حجم فایل: ۵۰ مگابایت</p>
            </div>
          </div>
          <label className="block">
            <span className="mb-2 block text-xs text-ink-300">کلید مدیریت</span>
            <input
              className="input-ay"
              type="password"
              value={token}
              onChange={(e) => setToken(e.target.value)}
              placeholder="ARTISTYAR_UPLOAD_ADMIN_TOKEN"
              autoComplete="off"
              required
            />
          </label>
          <label className="block">
            ...
            <select className="input-ay" name="category" defaultValue="prodby-mehrshad">
              <option value="prodby-mehrshad">ProdBy Mehrshad (نمونه‌کار خودم)</option>
          ...
          <label className="block">
            ...
            <input className="input-ay" name="title" placeholder="مثلاً میکس تنظیم…" minLength={3} required />
          </label>
          <label className="block">
            <span className="mb-2 block text-xs text-ink-300">توضیحات</span>
            <textarea className="input-ay min-h-28 resize-y" name="description" placeholder="توضیح کوتاه درباره محتوا…" />
          </label>
          <label className="block">
            <span className="mb-2 block text-xs text-ink-300">فایل صوتی، ویدیویی یا تصویری</span>
            <input
        ...
        <div className="card-ay p-6">
          <div className="flex items-center gap-3 border-b border-white/[.07] pb-4">
            ...
            <div>
              <h3 className="font-medium text-sand-50">محتوای آپلودشده</h3>
              <p className="mt-1 text-xs text-ink-500">ویرایش، استخراج کاور MP3 یا حذف</p>
            </div>
          </div>
            ...
            {items.length ? (
              items.map((item) => (
                <div key={item.id} className="rounded-xl border border-white/[.07] bg-white/[.02] p-3">
                  ...
                  <a className="mt-2 block truncate text-xs text-ink-500 hover:text-gold-300" href={item.url} target="_blank" rel="noreferrer">
                    مشاهده فایل
                  </a>
              ...
              <p className="text-sm leading-7 text-ink-500">هنوز محتوایی از Supabase Storage دریافت نشده است.</p>
            )}
          </div>
        </div>
      </div>
      <div className="card-ay p-6">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[.07] pb-4">
          <div>
            <h3 className="font-medium text-sand-50">انتخاب از فایل‌های موجود (شامل ProdBy Mehrshad)</h3>
            <p className="mt-1 text-xs text-ink-500">
              فایل‌هایی که مستقیم در bucket آپلود کرده‌ای — از جمله فولدر ProdBy Mehrshad — بدون آپلود دوباره اینجا ثبت کن.
            </p>
          </div>
          <button type="button" className="btn-ghost !px-3 !py-1.5 text-xs" onClick={() => void loadStorage()} disabled={!token || busy}>
            بارگذاری فایل‌ها
          </button>
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {storageItems.length ? (
            ...
            <p className="text-sm leading-7 text-ink-500">
              برای دیدن فایل‌های موجود (از جمله ProdBy Mehrshad)، کلید مدیریت را وارد و روی «بارگذاری فایل‌ها» کلیک کن.
            </p>
          )}
        </div>
      </div>
      ...
      {editing ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-label="ویرایش محتوا">
