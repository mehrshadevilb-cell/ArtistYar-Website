...
async function fetchJson(url: string, timeoutMs: number) {
  ...
  try {
    ...
    let body: any = null;
    try { body = text ? JSON.parse(text) : null; } catch { throw new Error(`پاسخ نامعتبر از ${url}`); }
...
export default function AdminAnalyticsPage() {
  ...
  const load = useCallback(async () => {
    ...
    // Load the real analytics snapshot first. AI diagnostics must never block
    // the dashboard or make the whole page look broken.
    try {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Card title={`بازدید ${days} روزه`} value={fmt(s?.traffic?.page_views)} icon={Eye} sub={s?.traffic?.page_view_change_percent != null ? `تغییر: ${s.traffic.page_view_change_percent}%` : ""} /><Card title="بازدیدکننده یکتا" value={fmt(s?.traffic?.unique_visitors)} icon={Users} /><Card title="گفت‌وگوی AI" value={fmt(s?.ai?.website_chats)} icon={Bot} /><Card title="خطاهای AI" value={fmt(s?.ai?.errors)} icon={Activity} /></div>
    <div className="grid gap-4 lg:grid-cols-2"><section className="card-ay p-5"><h3 className="mb-5 flex gap-2 text-sm text-sand-50"><Gauge size={17} className="text-gold-400" />صفحه‌های پربازدید</h3><Bars items={(s?.traffic?.top_paths || []).slice(0, 12)} /></section><section className="card-ay p-5"><h3 className="mb-5 flex gap-2 text-sm text-sand-50"><Activity size={17} className="text-gold-400" />رویدادهای سایت</h3><Bars items={(s?.events || []).slice(0, 12)} /></section></div>
    <div className="grid gap-4 lg:grid-cols-3"><section className="card-ay p-5"><h3 className="text-sm text-sand-50">منابع ورودی / UTM</h3><div className="mt-5"><Bars items={(s?.traffic?.top_attribution || []).slice(0, 10).map((x: any) => ({ label: x.source, count: x.count }))} /></div></section><section className="card-ay p-5"><h3 className="text-sm text-sand-50">دستگاه‌ها</h3><div className="mt-5"><Bars items={Object.entries(s?.traffic?.devices || {}).map(([label, count]) => ({ label, count }))} /></div></section><section className="card-ay p-5"><h3 className="text-sm text-sand-50">آموزش</h3><div className="mt-5 space-y-3 text-xs text-ink-400"><p>هنرجو: <b className="text-sand-50">{fmt(s?.education?.students)}</b></p><p>کلاس فعال: <b className="text-sand-50">{fmt(s?.education?.active_classes)}</b></p><p>ثبت‌نام فعال: <b className="text-sand-50">{fmt(s?.education?.active_enrollments)}</b></p></div></section></div>
    <section className="card-ay p-5"><h3 className="flex gap-2 text-sm text-sand-50"><MousePointerClick size={17} className="text-gold-400" />قیف رفتار و تبدیل</h3><div className="mt-5 grid gap-2 sm:grid-cols-3 lg:grid-cols-6">{funnel.map(([label, value], i) => <div key={String(label)} className="rounded-xl border border-white/10 p-3"><p className="text-[11px] text-ink-500">{i + 1}. {label}</p><p className="mt-2 text-xl font-semibold text-sand-50">{fmt(Number(value))}</p>{i > 0 && Number(funnel[i - 1][1]) > 0 && <p className="mt-1 text-[10px] text-gold-400">{(Number(value) / Number(funnel[i - 1][1]) * 100).toLocaleString("fa-IR", { maximumFractionDigits: 1 })}% مرحله قبل</p>}</div>)}</div></section>
