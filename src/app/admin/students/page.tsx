...
export default function AdminStudentsPage() {
  ...
  async function load() { setLoading(true); setError(""); try { const response = await fetch(`/api/rahyar/admin/students?q=${encodeURIComponent(q)}`, { cache: "no-store" }); const data = await response.json(); if (!response.ok) throw new Error(readableError(data.error || data.detail, "دریافت هنرجوها ناموفق بود.")); setStudents(data); } catch (cause) { setError(cause instanceof Error ? cause.message : "دریافت هنرجوها ناموفق بود."); } finally { setLoading(false); } }
  useEffect(() => { void load(); }, []);
