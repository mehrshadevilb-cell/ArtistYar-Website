...
export async function POST(request: Request) {
  ...
  if (contentLength > MAX_BODY_BYTES) {
    return NextResponse.json(
      { ok: false, error: "درخواست بیش از حد بزرگ است.", code: "validation", retryable: false, requestId: "hn-size", latencyMs: 0 },
      { status: 413 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: "JSON نامعتبر.", code: "validation", retryable: false, requestId: "hn-json", latencyMs: 0 },
      { status: 400 },
    );
  }
  ...
  if (!validated.ok) {
    return NextResponse.json(
      { ok: false, error: validated.error, code: "validation", retryable: false, requestId: "hn-val", latencyMs: 0 },
      { status: 400 },
    );
  }
