...
export async function POST(request: Request) {
  ...
  try {
    const system = ref
      ...
      : `تو مهندس میکس سخت‌گیر هستی. مثل Reference تحلیل کن. فارسی ساده و قابل‌اجرا. اعداد متریک واقعی‌اند — جعل نکن. سخت‌گیر باش. فقط JSON معتبر برگردان با کلیدهای: fileSummary, matchScore, descriptors, loudness, tonal, stereo, dynamics, clipping, compression, eq, mixBalance, roadmap, quickFixes, referenceTips, source.`;
    const prompt = ref
      ? `فایل کاربر: ${(file as File).name}\nمتریک کاربر: ${JSON.stringify(enriched)}\nرفرنس: ${refName || "reference"}\nمتریک رفرنس: ${JSON.stringify(ref)}\nژانر: ${genre}\nتمرکز: ${focus}\nتوضیح کاربر: ${notes || "—"}\n\nمقایسه سخت‌گیرانه با رفرنس بده. matchScore = نزدیکی به همین رفرنس (نه تعریف کلی). تفاوت Peak/RMS/Crest/انرژی/استریو را صریح بگو و roadmap را برای نزدیک شدن به رفرنس بنویس.`
    ...
    if (parsed && typeof parsed === "object") {
      ...
      const floor = strictScore(enriched, genre, ref);
