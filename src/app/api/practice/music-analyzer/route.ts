...
function fallback(m: any, genre: string, focus: string) {
  ...
  const score = strictScore(m, genre);

  return {
    fileSummary: `تحلیل سخت‌گیرانه برای «${genre}» با تمرکز «${focus}». Peak ${m.peakDbfs.toFixed(1)} dBFS · RMS ${m.rmsDbfs.toFixed(1)} dBFS · Crest ${m.crestFactorDb.toFixed(1)} dB.` + (m.approxLufs != null ? ` · ≈${m.approxLufs.toFixed(1)} LUFS` : ""),
    matchScore: score,
    descriptors: {
