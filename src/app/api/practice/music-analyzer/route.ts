...
function fallback(m: any, genre: string, focus: string) {
  ...
  return {
    fileSummary: `تحلیل سخت‌گیرانه برای «${genre}» با تمرکز «${focus}». Peak ${m.peakDbfs.toFixed(1)} dBFS · RMS ${m.rmsDbfs.toFixed(1)} dBFS · Crest ${m.crestFactorDb.toFixed(1)} dB.`,
    matchScore: score,
    descriptors: {
    ...
    loudness: {
      ...
      targetLufs: /edm|electro/i.test(genre) ? "حدود ۷- تا ۶- LUFS" : "حدود ۹- تا ۸- LUFS",
      truePeak: "≤ -1.0 dBTP",
    },
    tonal: {
      summary: `پایین ${low.toFixed(0)}٪ · میانی ${mid.toFixed(0)}٪ · بالا ${high.toFixed(0)}٪` + (m.spectralCentroidHz ? ` · مرکز حدود ${Math.round(m.spectralCentroidHz)}Hz` : ""),
    ...
    clipping: m.peakDbfs >= -0.3 ? "پیک نزدیک سقف است — فوری گین را اصلاح کن." : m.peakDbfs >= -1 ? "پیک نزدیک ۱- است؛ هدرووم کم." : "پیک نسبتاً امن است.",
    compression: {
      summary: "کمپرس را بعد از تعادل بزن؛ هدف کنترل قله‌هاست.",
    ...
    eq,
    mixBalance: [
      { element: "وکال", advice: "جلو باشد بدون تیز شدن." },
    ...
    quickFixes: ["اول گل‌آلودگی را کم کن", "ساب را مونو نگه دار", "لیمیتر فقط سقف بدهد", "با رفرنس A/B کن"],
    referenceTips: "مثل Reference: خط تعادل را دنبال کن. امتیاز پایین یعنی فاصله با استاندارد حرفه‌ای.",
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ ok: true, limit, used, remaining: Math.max(0, limit - used), pro, course, admin: false, tier: pro ? "pro" : course ? "course" : "free" });
}
...
export async function POST(request: Request) {
  ...
  try {
    const system = `تو مهندس میکس سخت‌گیر هستی. مثل Reference تحلیل کن. فارسی ساده و قابل‌اجرا. اعداد متریک واقعی‌اند — جعل نکن. سخت‌گیر باش. فقط JSON معتبر برگردان با کلیدهای: fileSummary, matchScore, descriptors, loudness, tonal, stereo, dynamics, clipping, compression, eq, mixBalance, roadmap, quickFixes, referenceTips, source.`;
    const prompt = `فایل: ${(file as File).name}\nمتریک: ${JSON.stringify(enriched)}\nژانر: ${genre}\nتمرکز: ${focus}\nمشکل: ${notes || "—"}\nتحلیل سخت‌گیرانه بده.`;
  ...
  return NextResponse.json({
    ...
    quota: {
      ...
      course,
      admin: isAdmin,
      tier: isAdmin ? "admin" : pro ? "pro" : course ? "course" : "free",
    },
  });
}
