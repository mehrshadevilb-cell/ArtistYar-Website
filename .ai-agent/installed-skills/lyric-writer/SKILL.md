...
- **STOP** and report: "This is an instrumental track — no lyrics needed. Use ``suno-engineer` skill` to create the Style Box directly."
...
3. **Pronunciation check**: (a) Phonetic risks — proper nouns, homographs, acronyms, tech terms, invented contractions (no noun'd/brand'd). (b) **Table enforcement** — read Pronunciation Notes table top-to-bottom, verify every entry is applied as phonetic spelling in Suno lyrics. See `this skill package root/reference/suno/pronunciation-guide.md` for full enforcement workflow.
...
*(Canonical homograph reference: `this skill package root/reference/suno/pronunciation-guide.md`. Keep this table in sync.)*
...
- Full homograph reference: `this skill package root/reference/suno/pronunciation-guide.md`
...
8. **Hand off to Suno engineer** - Automatically invoke ``suno-engineer` skill` with the track file path to populate the Style Box and Suno Inputs section. Do not wait for the user to request this — it is the natural next step after lyrics are finalized.
